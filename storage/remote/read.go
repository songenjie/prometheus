...
func (c *allQueryableClient) Querier(ctx context.Context, mint, maxt int64) (_ storage.Querier, err error) {
	q := &querier{
		ctx:              ctx,
		mint:             mint,
		maxt:             maxt,
	...
	var stillQuery bool
	q.maxt, stillQuery, err = c.preferLocalStorage(mint, maxt)
	if err != nil {
		return
	}
	if stillQuery {
...
func (c *allQueryableClient) ChunkQuerier(ctx context.Context, mint, maxt int64) (_ storage.ChunkQuerier, err error) {
	cq := &chunkQuerier{
		querier: querier{
			ctx:              ctx,
			mint:             mint,
	...
	var stillQuery bool
	cq.querier.maxt, stillQuery, err = c.preferLocalStorage(mint, maxt)
	if err != nil {
		return
	}
	if stillQuery {
...
func (c *allQueryableClient) preferLocalStorage(mint, maxt int64) (cmaxt int64, stillQuery bool, err error) {
	...
	return cmaxt, true, nil
}

// AllQueryableClient returns a storage.AllQueryable which queries the given Client to select series sets.
func AllQueryableClient(c *Client, externalLabels labels.Labels, requiredMatchers []*labels.Matcher, readRecent bool, callback startTimeCallback) storage.AllQueryable {
